...
-- drop next run table
DROP TABLE trigger_next_run;

-- drop master table first
DROP TABLE super_master_table;

---------------------------
-- drop the LVL1 tables

-- drop the master table first
DROP TABLE l1_master_table;

-- drop N-N tables
DROP TABLE l1_tm_to_ti;
DROP TABLE l1_ti_to_tt;
DROP TABLE l1_tt_to_ttv;
DROP TABLE l1_tm_to_tt;
DROP TABLE l1_tm_to_tt_forced;
DROP TABLE l1_bgs_to_bg;
DROP TABLE l1_bg_to_b;
DROP TABLE l1_ci_to_csc;

-- drop tables with FK
DROP TABLE l1_prescale_set;
DROP TABLE l1_trigger_menu;
-- drop the rest
DROP TABLE l1_bunch_group_set;
DROP TABLE l1_bunch_group;
DROP TABLE l1_muctpi_info;
DROP TABLE l1_dead_time;
DROP TABLE l1_random;
DROP TABLE l1_prescaled_clock;
DROP TABLE l1_jet_input;
DROP TABLE l1_calo_sin_cos; 
DROP TABLE l1_ctp_files;
DROP TABLE l1_ctp_smx;
DROP TABLE l1_trigger_type;
DROP TABLE l1_trigger_item;
DROP TABLE l1_trigger_threshold;
DROP TABLE l1_trigger_threshold_value;
DROP TABLE l1_calo_info;

------------------------------
-- drop the HLT tables

-- drop the master table first
DROP TABLE hlt_master_table;

-- drop the N-N tables
DROP TABLE hlt_cp_to_pa;
DROP TABLE hlt_so_to_py;
DROP TABLE hlt_so_to_dl;
DROP TABLE hlt_st_to_cp;
DROP TABLE hlt_re_to_dl;
DROP TABLE hlt_dl_to_en;
DROP TABLE hlt_tm_to_tc;
DROP TABLE hlt_ts_to_te;
DROP TABLE hlt_te_to_te;
DROP TABLE hlt_tc_to_ts;
-- DROP TABLE hlt_tc_to_st;
DROP TABLE hlt_te_to_cp;
-- DROP TABLE hlt_fd_to_st;
DROP TABLE hlt_tc_to_tr;

-- drop the tables with FK
DROP TABLE hlt_force_dll;
DROP TABLE hlt_source;
DROP TABLE hlt_prescale;
DROP TABLE hlt_prescale_set;
DROP TABLE hlt_trigger_signature;
DROP TABLE hlt_trigger_menu;
DROP TABLE hlt_trigger_chain;

-- drop the rest
DROP TABLE hlt_parameter;
DROP TABLE hlt_property;
DROP TABLE hlt_dll;
DROP TABLE hlt_release;
DROP TABLE hlt_environment;
DROP TABLE hlt_trigger_element;
DROP TABLE hlt_setup;
DROP TABLE hlt_component;
DROP TABLE hlt_trigger_stream;
...
CREATE TABLE l1_trigger_type (
	...
	l1tty_version		NUMBER(11,5)	NOT NULL,
...
CREATE TABLE l1_dead_time ( 
	...
	l1dt_version 		NUMBER(11,5)	NOT NULL,
...
CREATE TABLE l1_random (
	...
	l1r_version 		NUMBER(11,5)	NOT NULL,
	l1r_rate1 		NUMBER(10) 	NOT NULL,
	l1r_rate2 		NUMBER(10) 	NOT NULL,
	l1r_username		VARCHAR2(15)	,
...
CREATE TABLE l1_jet_input (
	...
	l1ji_version 		NUMBER(11,5)	NOT NULL,
...
CREATE TABLE l1_calo_sin_cos (
	...
	l1csc_version 		NUMBER(11,5)	NOT NULL,
...
CREATE TABLE l1_bunch_group_set (
	...
	l1bgs_version		NUMBER(11,5)	NOT NULL,
	l1bgs_username		VARCHAR2(15)	,
...
CREATE TABLE l1_bunch_group (
	...
	l1bg_version		NUMBER(11,5)	NOT NULL,
	l1bg_username		VARCHAR2(15)	,
...
CREATE TABLE l1_ctp_files ( 
	...
	l1cf_version 		NUMBER(11,5)	NOT NULL,
	l1cf_lut		CLOB		,
	l1cf_cam		CLOB		,
	l1cf_username		VARCHAR2(15)	,
...
CREATE TABLE l1_ctp_smx (
	...
	l1smx_version		NUMBER(11,5)	NOT NULL,
	l1smx_output		CLOB		,
...
CREATE TABLE l1_trigger_threshold_value (
	...
	l1ttv_version 		NUMBER(11,5)	NOT NULL,
	l1ttv_type		VARCHAR2(10)	NOT NULL,
	l1ttv_pt_cut 		NUMBER(10) 	NOT NULL,
	l1ttv_eta_min 		NUMBER(10) 	,
	l1ttv_eta_max 		NUMBER(10) 	,
	l1ttv_phi_min 		NUMBER(10) 	,
	l1ttv_phi_max 		NUMBER(10) 	,
	l1ttv_em_isolation 	NUMBER(10) 	,
	l1ttv_had_isolation 	NUMBER(10) 	,
	l1ttv_had_veto 		NUMBER(10) 	,
	l1ttv_window 		NUMBER(10) 	,
	l1ttv_priority		NUMBER(11,5)	,
	l1ttv_username		VARCHAR2(15)	,
...
CREATE TABLE l1_calo_info (
        ...
        l1ci_version                    NUMBER(11,5)    NOT NULL,
	l1ci_global_scale		NUMBER(10)	NOT NULL,
...
CREATE TABLE l1_tm_to_ti ( 
	...
	l1tm2ti_ctp_id			NUMBER(10) 	NOT NULL,
	l1tm2ti_username 		VARCHAR2(15)	,
...
-- (i.e. l1_tm_to_tt) must be comiled from l1_tm_to_ti and from the
...
CREATE TABLE l1_prescale_set (
	...
	l1ps_version		NUMBER(11,5)	NOT NULL,
	l1ps_lumi		NUMBER(11,5)	NOT NULL,
...
CREATE TABLE l1_master_table (
	...
	l1mt_version		NUMBER(11,5)	NOT NULL,
	...
	l1mt_trigger_type_id	NUMBER(10)	NOT NULL,
	l1mt_calo_info_id	NUMBER(10)	NOT NULL,
	l1mt_username		VARCHAR2(15)	,
	...
	CONSTRAINT		l1mt_fk_bgs	FOREIGN KEY (l1mt_bunch_group_set_id)
						REFERENCES l1_bunch_group_set(l1bgs_id),
	CONSTRAINT 		l1mt_fk_tty	FOREIGN KEY (l1mt_trigger_type_id)
						REFERENCES l1_trigger_type(l1tty_id),
...
CREATE TABLE hlt_parameter (
	...
	hpa_username		VARCHAR2(15)	,
	hpa_modified_time	TIMESTAMP	,
	hpa_used		CHAR		NOT NULL,
...
CREATE TABLE hlt_component (
	...
	hcp_name		VARCHAR2(150) 	NOT NULL,
	hcp_version	NUMBER(11,5)	NOT NULL,
	hcp_alias		VARCHAR2(150) 	NOT NULL,
	hcp_topalg  NUMBER(1) 	NOT NULL,
	hcp_flag		VARCHAR2(50)	,
	hcp_username		VARCHAR2(15)	,
	hcp_modified_time	TIMESTAMP	,
	hcp_used		CHAR		NOT NULL,
...
CREATE TABLE hlt_cp_to_pa (
	...
	hcp2pa_username		VARCHAR2(15)	,
	hcp2pa_modified_time	TIMESTAMP	,
	hcp2pa_used		CHAR		NOT NULL,
...
CREATE TABLE hlt_setup (
	...
	hst_version		NUMBER(11,5)	NOT NULL,
	hst_username		VARCHAR2(15)	,
	hst_modified_time	TIMESTAMP	,
	hst_used		CHAR		NOT NULL,
...
CREATE TABLE hlt_st_to_cp (
	...
	hst2cp_username		VARCHAR2(15)	,
	hst2cp_modified_time	TIMESTAMP	,
	hst2cp_used		CHAR		NOT NULL,
...
-- This table stores all information about all dlls in all Trigger releases.
CREATE TABLE hlt_dll (
	hdl_id			NUMBER(10) AUTO_INCREMENT NOT NULL,
	hdl_name 		VARCHAR2(50)	NOT NULL,
	hdl_package 		VARCHAR2(50)	NOT NULL,
	hdl_cmt_version		VARCHAR2(60)	NOT NULL,
	hdl_username		VARCHAR2(15)	,
	hdl_modified_time	TIMESTAMP	,
	hdl_used		CHAR		NOT NULL,
	CONSTRAINT		hdl_pk		PRIMARY KEY (hdl_id),
	CONSTRAINT 		hdl_nmver	UNIQUE	(hdl_name, hdl_package, hdl_cmt_version)
	);

-- This table contains the relation between all 'runnable' components above
-- and their real meat, that is, the name of the type and the library that
-- actually contains the component. The DLL library name should be available
-- in a given software setup (see masterkey) or a runtime error will
-- be generated. There is, nevertheless, *no* linkage between this table and
-- the dll table since we would like to have a setup independent of changes
-- in the software releases. The component origin and DLL remains more or less
-- fixed from release to release and there is no need to change it every time.
-- A consistency check has to be performed after a masterkey selection (or at
-- (DB data input) to understand if a given set of components chosen to run 
-- have a set of 'source' DLL's that are available for the software release
-- chosen.
CREATE TABLE hlt_source (
	hso_id			NUMBER(10) AUTO_INCREMENT NOT NULL,
	hso_name		VARCHAR2(50) 	NOT NULL,
	hso_gaudi_type		VARCHAR2(50)	NOT NULL,
-- hso_dll_id is to be removed and replaced by table hlt_so_to_dl
-- temporarily left in for backward compatibility
--	hso_dll_id		NUMBER(10)	NOT NULL,
	hso_username		VARCHAR2(15)	,
	hso_modified_time	TIMESTAMP	,
	hso_used		CHAR		NOT NULL,
-- the following constraint can go in once hso_dll_id is replaced so that duplicates
-- need no longer go into hlt_source
--	CONSTRAINT		hso_nmtype	UNIQUE	(hso_name, hso_gaudi_type),
	CONSTRAINT		hso_pk		PRIMARY KEY (hso_id)
	);

-- This table makes the n-to-n relationship between the dlls and sources.
CREATE TABLE hlt_so_to_dl (
	hso2dl_id		NUMBER(10) AUTO_INCREMENT NOT NULL,
	hso2dl_source_id	NUMBER(10)	NOT NULL,
	hso2dl_dll_id		NUMBER(10) 	NOT NULL,
	CONSTRAINT 		hso2dl_pk 	PRIMARY KEY (hso2dl_id),
	CONSTRAINT 		hso2dl_fk_so	FOREIGN KEY (hso2dl_source_id)
	 					REFERENCES hlt_source(hso_id),
	CONSTRAINT 		hso2dl_fk_dl	FOREIGN KEY (hso2dl_dll_id)
	 					REFERENCES hlt_dll(hdl_id)
	);

-- This table defines all properties for the dll sources.
CREATE TABLE hlt_property (
	hpy_id			NUMBER(10) AUTO_INCREMENT NOT NULL,
	hpy_name		VARCHAR2(50)	NOT NULL,
	hpy_type		VARCHAR2(50)	NOT NULL,
	hpy_value		VARCHAR2(4000)	,
	hpy_username		VARCHAR2(15)	,
	hpy_modified_time	TIMESTAMP	,
	hpy_used		CHAR		NOT NULL,
	CONSTRAINT		hpy_pk		PRIMARY KEY (hpy_id)
	);

-- This table makes the n-to-n relationship between the dll sources and the 
-- properties.
CREATE TABLE hlt_so_to_py (
	hso2py_id		NUMBER(10) AUTO_INCREMENT NOT NULL,
	hso2py_source_id	NUMBER(10)	NOT NULL,
	hso2py_property_id	NUMBER(10) 	NOT NULL,
	CONSTRAINT 		hso2py_pk 	PRIMARY KEY (hso2py_id),
	CONSTRAINT 		hso2py_fk_so	FOREIGN KEY (hso2py_source_id)
	 					REFERENCES hlt_source(hso_id),
	CONSTRAINT 		hso2py_fk_py	FOREIGN KEY (hso2py_property_id)
	 					REFERENCES hlt_property(hpy_id)
	);
...
CREATE TABLE hlt_release (
	...
	hre_username		VARCHAR2(15)	,
	hre_modified_time	TIMESTAMP	,
	hre_used		CHAR		NOT NULL,
	CONSTRAINT		hre_pk		PRIMARY KEY (hre_id),
	CONSTRAINT		hre_namever 	UNIQUE (hre_name, hre_version)
	);

-- This table stores the N:N relationship that exists between releases and
-- the libraries they contain. A particular DLL can be included in many
-- software releases and a particular software release can contain a number
-- of libraries.
CREATE TABLE hlt_re_to_dl (
	hre2dl_id		NUMBER(10) AUTO_INCREMENT NOT NULL,
	hre2dl_release_id	NUMBER(10) 	NOT NULL,
	hre2dl_dll_id		NUMBER(10) 	NOT NULL,
	hre2dl_username		VARCHAR2(15)	,
	hre2dl_modified_time	TIMESTAMP	,
	hre2dl_used		CHAR		NOT NULL,
	CONSTRAINT 		hre2dl_pk 	PRIMARY KEY (hre2dl_id),
	CONSTRAINT		hre2dl_fk_dll	FOREIGN KEY (hre2dl_dll_id)
	 					REFERENCES hlt_dll(hdl_id),
	CONSTRAINT		hre2dl_fk_re	FOREIGN KEY (hre2dl_release_id)
	 					REFERENCES hlt_release(hre_id)
	);

-- This table contains environment variables for the DLLs that need to be set
-- so the DLLs can work properly. This should be probably parsed by a script
-- to setup the process environment in which the HLT processor has to run.
CREATE TABLE hlt_environment (
	hen_id			NUMBER(10) AUTO_INCREMENT NOT NULL,
	hen_name		VARCHAR2(20)	NOT NULL,
	hen_version		NUMBER(11,5)	NOT NULL,
	hen_value 		VARCHAR2(20)	NOT NULL,
	hen_username		VARCHAR2(15)	,
	hen_modified_time	TIMESTAMP	,
	hen_used		CHAR		NOT NULL,
	CONSTRAINT		hen_pk		PRIMARY KEY (hen_id),
	CONSTRAINT		hen_nmver	UNIQUE (hen_name, hen_version)
	);

-- This table makes the n-to-n relationship between the dll and the 
-- dll_parameters.
CREATE TABLE hlt_dl_to_en (
	hdl2en_id		NUMBER(10) AUTO_INCREMENT NOT NULL,
	hdl2en_dll_id		NUMBER(10)	NOT NULL,
	hdl2en_environment_id	NUMBER(10) 	NOT NULL,
	CONSTRAINT 		hdl2en_pk 	PRIMARY KEY (hdl2en_id),
	CONSTRAINT 		hdl2en_fk_dl	FOREIGN KEY (hdl2en_dll_id)
	 					REFERENCES hlt_dll(hdl_id),
	CONSTRAINT 		hdl2en_fk_en	FOREIGN KEY (hdl2en_environment_id)
	 					REFERENCES hlt_environment(hen_id)
	);
...
CREATE TABLE hlt_trigger_menu ( 
	...
	htm_version 		NUMBER(11,5)	NOT NULL,
	htm_phase 		VARCHAR2(20) 	NOT NULL,
	htm_shift_safe		CHAR		NOT NULL,
	htm_username		VARCHAR2(15)	,
...
CREATE TABLE hlt_trigger_chain (
	...
	CONSTRAINT		htc_fk_st	FOREIGN KEY (htc_setup_id) 
						REFERENCES hlt_setup(hst_id)
	);
...
CREATE TABLE hlt_trigger_element ( 
	...
	hte_name          VARCHAR2(30)  NOT NULL,
	hte_version       NUMBER(11,5)  NOT NULL,		
	hte_username      VARCHAR2(15),
...
CREATE TABLE hlt_prescale_set ( 
	...
	hps_version 		NUMBER(11,5)	NOT NULL,
	hps_trigger_menu_id	NUMBER(10)	NOT NULL,
	hps_username		VARCHAR2(15)	,
...
CREATE TABLE hlt_tm_to_tc ( 
	...
	htm2tc_chain_counter	NUMBER(10)	NOT Null,
	htm2tc_username 	VARCHAR2(15)	,
...
CREATE TABLE hlt_tc_to_tr (
  ...
  CONSTRAINT htc2tr_fk_tm   FOREIGN KEY (htc2tr_trigger_stream_id) REFERENCES hlt_trigger_stream(hts_id)
);
...
CREATE TABLE hlt_trigger_stream (
  hts_id                 NUMBER(10)   NOT NULL,
	hts_stream_letter      VARCHAR2(3),
	hts_stream_description VARCHAR2(200),
	hts_username           VARCHAR2(15),
	hts_modified_time      TIMESTAMP,
	hts_used               CHAR         NOT NULL,
	CONSTRAINT hts_pk      PRIMARY KEY (hts_id)
...
CREATE TABLE hlt_te_to_te (
	...
	hte2te_trigger_element_id	NUMBER(10)	NOT NULL,
	hte2te_trigger_element_inp_id	VARCHAR2(30)	NOT NULL,
	hte2te_username 		VARCHAR2(15)	,
	hte2te_modified_time		TIMESTAMP	,
	hte2te_used			CHAR		NOT NULL,
	CONSTRAINT			hte2te_pk	PRIMARY KEY (hte2te_id),
	CONSTRAINT			hte2cte_fk_te	FOREIGN KEY (hte2te_trigger_element_id) 
							REFERENCES hlt_trigger_element(hte_id)
	);
...
CREATE TABLE hlt_master_table (
	...
	hmt_version 		NUMBER(11,5)		NOT NULL,
	hmt_prescale_set_id	NUMBER(10)		,
	hmt_trigger_menu_id	NUMBER(10)		,
	hmt_forced_setup_id	NUMBER(10)		,
	hmt_release_id		NUMBER(10)		NOT NULL,
	hmt_username 		VARCHAR2(15)		,
